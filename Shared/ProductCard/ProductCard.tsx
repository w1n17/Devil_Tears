...
export default function ProductCard({
  ...
  return (
    <div className={`relative ${isActive ? "z-50" : "z-0"}`}>
      ...
      >
        <div
          className={`bg-gradient-to-tr from-[#544545] from-50% to-[#B84747] rounded-3xl transition-all duration-300 relative ${
            isActive
              ? "w-full max-w-[800px] h-96 mx-4 p-8"
              : "w-64 h-96 m-4 hover:scale-105"
          }`}
        ...
        >
          {isActive && (
            <>
              <button
                ...
                className="absolute top-4 left-4 p-2 bg-white/90 rounded-full hover:bg-red-500 transition-colors z-10"
              >
                <img
                  src={
                    isFavorite
              ...
              <button
                className="absolute top-4 right-4 text-white text-3xl z-[100] hover:text-red-400 transition-colors"
                onClick={(e) => {
                  e.stopPropagation();
                  onClick();
                }}
          ...
          <div
            className={`flex ${
              isActive ? "flex-row h-full gap-8" : "flex-col h-full"
            }`}
          >
            {!isActive ? (
              ...
              <>
                ...
                />
                <div className="w-1/2 text-left flex flex-col justify-center gap-6">
                  <div className="flex items-baseline gap-2">
                    <span className="text-white text-2xl font-alegreya">
                      {category}
                    </span>
                    <h1 className="text-2xl text-white font-alegreya">
                      {name}
                    </h1>
                  </div>
                  <div className="space-y-6">
                    <p className="text-base text-white font-alegreya">
                      {description}
                    </p>
                    ...
                    <div className="flex flex-col gap-2">
                      <span className="text-2xl text-white font-alegreya">
                        {price}
                      </span>
                      <button
                        className="mt-4 bg-white text-black px-6 py-3 w-52 rounded-full font-alegreya hover:bg-opacity-90 transition"
